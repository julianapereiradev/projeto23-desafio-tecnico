...
async function getBetsByGame(gameId: number) {
  ...
  if (!betsByGame)
    throw notFoundException("This game ID does not exist in the bank");
...
async function calculateAndUpdateBets(
  gameId: number,
  finalScore: FinalScoreProtocol
) {
  ...
  for (const bet of bets) {
    ...
    const wonAmount = isCorrectBet
      ? Math.floor(
          (amountBet / totalWinningAmount) * totalBetAmount * (1 - houseFee)
        )
      : 0;

    await gamesRepository.updateBetStatusAndAmountWon(
      bet.id,
      isCorrectBet ? "WON" : "LOST",
      wonAmount
    );
